...
function createRegistry({ now, setTimeout: setT, clearTimeout: clearT, randomInt, randomBytes, config, onChange = () => {}, onDelete = () => {} }) {
  ...
  // Drop a seat and cancel its expiry timer; a timer left armed would fire against a room whose
  // seat map no longer has it (harmless today, but it keeps the room object referenced).
  function removeSeat(room, id) {
  ...
  return { rooms, reserveCode, get, getOrCreate, delete: remove, join, resume, disconnect, attachVisitor, detachVisitor, addBot, removeBot, removeSeat, sweep, occupantsConnected };
}
