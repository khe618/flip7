...
- Test runner is `node --test` (`npm test`). Tests use `node:test` + `node:assert/strict`. Unit tests never sleep: every `lib/` factory takes injected `now / setTimeout / clearTimeout / random`. The two integration tests (`tests/server.test.js`, `tests/live.test.js`) spawn the real server with `PORT=0`, parse the port from its log line, and bound every wait with a deadline.
- **Every code block in this plan was extracted and run as a whole on 2026-09-18: all 92 tests passed.** Transcribe the blocks exactly; if a test fails after faithful transcription, suspect the transcription first.
...
- Produces: `createGame({ players: [{id, name}], seed, targetScore = 200, deck = null }) -> state`; `step(state, input) -> { state, events }` with inputs `{ type: "start_round" }` and `{ type: "act", player, action }`; `legalActions(state) -> string[]`; `pendingPlayer(state) -> id | null`; `class IllegalAction extends Error`. State shape is spec §4.3 (`round.lines`, `round.dealCursor`, `round.turnSeat`, `round.pending`, `round.resolution`, `round.results`; top-level `deck`, `discard`, `history`, `winner`, `phase`). Resolution frames carry one extra boolean, `ended`: a `flip_three` frame whose three flips are done stays on the stack as a continuation and feeds its `setAside` cards to the stack one at a time, so the public stack never shows a set-aside card as a frame before its turn.
- Exhaustion ruling (spec §2.3): when the deck is empty and the discard pile holds only action cards, the flip is skipped (`deck_exhausted`) instead of reshuffling, because reshuffling only action cards would re-flip them forever. Unreachable with the real deck; keeps tiny test decks finite.
...
- `decide` also returns `request`: the actual envelope of the first attempt (logged by the runner).
- Subprocess ruling: a stdout line without a `request_id` (non-JSON, or JSON missing the field) is attributed to the single pending request when exactly one is pending and is then judged invalid; when nothing is pending it is discarded. Agents should always echo `request_id`. This is stated in `docs/agent-protocol.md` (Task 16).
...
- Produces:
  ...
  - Log record kinds in `games.jsonl`, in order: `run` header, then per game `game_start`, `decision`*, `game_end`, then `summary` footer. `game_start` is `{ kind, game_id, seed, rotation, seats: [{ id, name, seat, agent, spec }] }`. `decision` is `{ kind, game_id, turnNumber, player, agent, request: <the first attempt's actual envelope>, attempts, action, fallback_used, latency_ms }`. `game_end` is `{ kind, game_id, final_scores, winner, rounds, events }`. Busts and Flip 7s are counted from the events of **every** engine step (deals and round starts included), via the `account()` helper.
...
- Reference only: `C:\dev\follow-suit\lib\rooms.js` is the ancestor of this module; the complete implementation is below, so nothing needs to be ported.

**Interfaces:**
- Produces `createRegistry({ now, setTimeout, clearTimeout, randomInt, randomBytes, config, onChange, onDelete }) -> registry` (the registry never touches the engine; the server owns `room.game`) with:
...
`lib/rooms.js` (a port of follow-suit's registry with the occupant rule, `isAgent`, and thrown `{ code }` errors):
...
- Server logs `flip7 running at http://localhost:<port>` on listen, where `<port>` is the bound port (`server.address().port`, so `PORT=0` works); the tests parse it. An unknown resume token always answers `unknown_token`. On `uncaughtException`/`unhandledRejection` the process sets `exitCode = 1`, closes sockets with 1012, and force-exits after 500 ms on a referenced timer.
...
- Produces `driveLiveSeat({ url, room, spec, name, log }) -> { close(): Promise, done: Promise<result>, stats }` and a CLI `node bench/live.js --url ws://localhost:3000 --room abcd --agent <spec> [--name my-agent]`. Behaviour: `hello` once; connect to `<url>/ws?room=<room>`; send `join { name, agent: true }`; call the adapter's `start` on the first playing state of each game and `end` on `game_over`; on every `state` where `game.decision` is set and `game.current_player === you` and `game.turnNumber` has not been acted on, call `decide` with `timeoutMs = max(1, timer.remainingMs − 250)` (retry disabled) and send `act { turnNumber, action }`. States that arrive while a decision is in flight are kept and the newest is reprocessed afterwards, so a turn the server defaulted is never answered late. Reconnect on close except 4000 (backoff as the browser); `close()` cancels the reconnect timer and awaits `shutdown`. `done` resolves with the `game_over` state and rejects if `hello` fails.
...
- Deviations from the spec text, all recorded in the spec's decisions log on 2026-09-18: the game view adds `phase`, `current_player`, `results`, and `winner` (public; the browser needs them); resolution frames carry `ended` and set-asides become frames one at a time; the exhaustion ruling for an action-only discard; `game_start.seats` rows carry `{ id, name, seat, agent, spec }`; `createGame` accepts a test-only `deck`; the round-summary auto-advance lives in `room-game`.
- Codex adversarial review of the plan (2026-09-18) produced 20 findings; 19 were applied (quick-play send queue, live driver lifecycle and in-flight state handling, runner init inside the guarded lifecycle, set-aside continuation, deal-time event accounting, subprocess exit/hello handling, adapter hook deadlines, HTTP 200 checks, crash exit code, final-round totals in the server test, nearest-rank percentile, `unknown_token`, `seats` naming, four missing rule traces, `PORT=0` integration tests, card-flip `.new` marking, complete registry code, shell quoting). The one not applied: treating id-less subprocess replies as unmatchable; the ruling above attributes them to the sole pending request instead, because a non-JSON reply must count as an invalid attempt.
