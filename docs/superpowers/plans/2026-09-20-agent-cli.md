...
**Spec:** `docs/superpowers/specs/2026-09-20-agent-cli-design.md`

## Global Constraints

- Node `>=22.12`; CommonJS with `"use strict"`; no build step; no new dependencies.
- Tests run with `npm test` (`node --test`), never sleep on real timers except where the existing adapter tests already do (fixture processes), and must pass on Windows (this machine) with Unix branches guarded by `process.platform`.
...
| `tests/fixtures/fake-claude.js` (new) | Stand-in for `claude` in tests. |
...
| `package.json`, `LICENSE`, `.gitattributes`, `README.md`, `docs/agent-protocol.md`, `public/index.html` | Packaging and docs. |
...
**Interfaces:**
- Produces: `parseRoom(input, defaultUrl) -> { url, room }`; `parseArgs(argv, { defaultUrl }) -> { url, room, spec, name, quiet, help, version }`; `class UsageError extends Error`; `USAGE` string; `DEFAULT_URL = "wss://flip7-arena.onrender.com"`.
...
test("parseArgs: positional room, --room alias, --url only applies to bare codes", () => {
...
function parseRoom(input, defaultUrl) {
  ...
  const proto = { "https:": "wss:", "wss:": "wss:", "http:": "ws:", "ws:": "ws:" }[u.protocol];
...
**Interfaces:**
- Produces: `resolveExecutable(name, { env, platform }) -> { path, shim } | null`; `spawnDirect(exe, args, opts) -> ChildProcess` where `exe` is a `resolveExecutable` result; `killTree(child)`; `quoteWindowsArg(a)`; `SHIM_UNSAFE` regex.
...
test("resolveExecutable finds node itself on the real PATH and returns null for a missing name", () => {
...
test("resolveExecutable honours PATHEXT on Windows and flags .cmd shims", () => {
  ...
  const abs = resolveExecutable(path.join(dir, "tool.cmd"), { env: {}, platform: "win32" });
  assert.equal(abs.shim, true, "an explicit path is accepted without a PATH search");
});
...
test("spawnDirect refuses shim-unsafe arguments and runs a direct executable", async () => {
  ...
  assert.equal(out, "a b|", "spaces and empty arguments survive a direct spawn");
...
test("killTree ends a sleeping child and its descendants", async () => {
  ...
  const grandPid = Number(await new Promise((r) => child.stdout.once("data", (b) => r(String(b)))));
  killTree(child);
  await new Promise((r) => child.on("close", r));
  // Poll for the grandchild's death: signal 0 throws ESRCH once it is gone.
...
function resolveExecutable(name, { env = process.env, platform = process.platform } = {}) {
  ...
  for (const dir of dirs) for (const ext of exts) {
    ...
    if (isFile(p)) return { path: p, shim: shim(p) };
  }
  return null;
}
...
// taskkill /t walks the tree. A failed taskkill spawn (not on PATH) emits "error" on its own
// process object; without a listener that is an unhandled error during shutdown.
function killTree(child) {
  if (!child || child.pid === undefined || child.exitCode !== null || child.signalCode !== null) return;
  if (process.platform === "win32") { spawn("taskkill", ["/pid", String(child.pid), "/t", "/f"], { stdio: "ignore", windowsHide: true }).on("error", () => {}); return; }
...
Run: `node --test tests/spawn.test.js`
Expected: all 5 pass on Windows. (`killTree` on Windows uses taskkill; the grandchild poll must report dead.)

- [ ] **Step 5: Switch `subprocess.js` to the shared helper**

In `agents/adapters/subprocess.js`: delete the `killChild` function and its comment block (lines starting `// spawn() always runs \`command\`` through the closing `}` of `killChild`), add `const { killTree } = require("./spawn");` after the other requires, replace the one call `killChild()` inside `shutdown()` with `killTree(child)`, and change the spawn line in `hello()` to:

```js
      child = spawn(command, { shell: true, stdio: ["pipe", "pipe", "pipe"], detached: process.platform !== "win32", windowsHide: true });
```

Add above it: `// detached on Unix so killTree can SIGKILL the whole group (sh and everything it started).`
...
- Produces: `create(spec, { command, model }) -> adapter` with `{ spec, name, version, hello, start, move, end, shutdown }`. `command` is a string (executable name or path) or an array `[exe, ...leadingArgs]`; default `process.env.FLIP7_CLAUDE_COMMAND || "claude"`. `resolveAgent("claude-code")` and `resolveAgent("claude-code:<model>", opts)` route here; `opts.command` is forwarded.
...
//   sleep         answers after 5 s (the adapter must kill it first)
...
test("claude-code: move sends the rendered request on stdin, passes --model, strips CLAUDECODE, and plays a game", async () => {
  ...
  delete process.env.CLAUDECODE; delete process.env.FAKE_CLAUDE_MODE;
});

test("claude-code: is_error, non-JSON, exit 1, and oversize output are invalid replies; three in a row are fatal", async () => {
  for (const mode of ["error", "nonjson", "exit1", "huge"]) {
    const a = claude("claude-code", mode);
    await a.hello();
    const d1 = await decide(a, { gameView: firstView(), gameId: "t", timeoutMs: 5000, retry: false });
    assert.equal(d1.fallback_used, true, mode);
    assert.equal(d1.attempts[0].outcome, "invalid", mode);
    const d2 = await decide(a, { gameView: firstView(), gameId: "t", timeoutMs: 5000, retry: false });
    assert.equal(d2.fallback_used, true, mode);
    await assert.rejects(decide(a, { gameView: firstView(), gameId: "t", timeoutMs: 5000, retry: false }), (err) => err instanceof AdapterError && /3 consecutive/.test(err.message), mode);
    await a.shutdown();
  }
  delete process.env.FAKE_CLAUDE_MODE;
});
...
test("claude-code: a good reply resets the consecutive-error counter", async () => {
  const counter = path.join(require("node:os").tmpdir(), `flip7-fake-claude-${process.pid}.txt`);
  try { require("node:fs").unlinkSync(counter); } catch { /* fresh */ }
  const a = claude("claude-code", "error-twice", { FAKE_CLAUDE_COUNTER: counter });
  await a.hello();
  const d1 = await decide(a, { gameView: firstView(), gameId: "t", timeoutMs: 5000, retry: false });
  const d2 = await decide(a, { gameView: firstView(), gameId: "t", timeoutMs: 5000, retry: false });
  const d3 = await decide(a, { gameView: firstView(), gameId: "t", timeoutMs: 5000, retry: false });
  const d4 = await decide(a, { gameView: firstView(), gameId: "t", timeoutMs: 5000, retry: false });
  assert.deepEqual([d1, d2, d3, d4].map((d) => d.fallback_used), [true, true, false, false]);
  await a.shutdown();
  delete process.env.FAKE_CLAUDE_MODE; delete process.env.FAKE_CLAUDE_COUNTER;
});

test("claude-code: a slow reply is killed at the deadline, and shutdown kills a live move", async () => {
  const a = claude("claude-code", "sleep");
  await a.hello();
  const d = await decide(a, { gameView: firstView(), gameId: "t", timeoutMs: 300, retry: false });
  assert.equal(d.attempts[0].outcome, "timeout");
  assert.equal(d.fallback_used, true);
  // A second move starts a fresh child; shutdown during it must return within ~2 s with no orphan.
  const pending = decide(a, { gameView: firstView(), gameId: "t", timeoutMs: 10000, retry: false });
...
function create(spec, { command = process.env.FLIP7_CLAUDE_COMMAND || "claude", model = null } = {}) {
  ...
  let exe = null, active = null, consecutiveErrors = 0;
  ...
  // (tree killed) or AdapterError (could not spawn).
  function run(argv, input, timeoutMs) {
    return new Promise((resolve, reject) => {
      ...
      child.on("error", (err) => { if (settled) return; settled = true; clearTimeout(timer); active = null; reject(new AdapterError(`${spec}: ${err.message}`)); });
      child.on("close", (code) => { if (active === child) active = null; if (settled) return; settled = true; clearTimeout(timer); resolve({ code, stdout, stderr }); });
    });
  }
  ...
  function parseResult(r) {
    ...
    const ok = j && r.code === 0 && j.is_error !== true && typeof j.result === "string";
...
Also update the final error message to `(expected claude-code, claude-code:<model>, bot:, file:, cmd:, or http://)`.

Note for `opts.command`: when undefined, `create` falls back to its default via destructuring default (`command = ...` applies to `undefined`). Keep it that way.

- [ ] **Step 6: Run to verify they pass**

Run: `node --test tests/adapters.test.js`
Expected: all pass (existing 9 plus 5 new). If the "huge" case fails on parse, confirm `cap` truncates to 64 KB and that `parseResult` sees non-JSON.
...
- Produces: `driveLiveSeat({ url, room, spec, name, log, onEvent, _send })` returning `{ done, stats, close(), _receive(msg), _handleState(msg), _token() }`; `class DriverError extends Error { code }` with codes `seat_taken_over | room_full | adapter`; `exitCodeFor(err) -> 2 | 3 | 4`; events `ready | seated | waiting | decision | game_over | reconnecting | error` as in spec §5.

- [ ] **Step 1: Update the existing unit test and add the new ones**

Replace `tests/live-unit.test.js` with:
...
const { AdapterError } = require("../agents/adapter");
...
function make(extra = {}) {
  ...
  const driver = driveLiveSeat({ url: "ws://127.0.0.1:1", room: "unit", spec: FIXTURE_SPEC, name: "unit", onEvent: (e) => events.push(e), _send: (m) => sent.push(m), ...extra });
...
function decisionState(turnNumber) {
  const s = playingState(turnNumber);
  s.game.decision = { type: "hit_or_stay" }; s.game.legal_actions = ["hit", "stay"]; s.game.current_player = "p1"; s.timer = { remainingMs: 2000 };
  s.game.you = "p1"; s.game.target_score = 200; s.game.dealer = "p1"; s.game.deck_remaining = 80; s.game.discard = []; s.game.resolution = []; s.game.history = []; s.game.history_start = 0;
...
test("an unseated visitor ignores playing and game_over, then joins exactly once at the next lobby after game_in_progress", async () => {
  ...
  await driver._receive(lobby);
  assert.deepEqual(sent.filter((m) => m.type === "join").length, 1, "one join per lobby, never a loop");
...
test("unknown_token clears identity and requests a fresh seat; a state with you:null drops the seat", async () => {
  ...
  assert.equal(sent.filter((m) => m.type === "join").length, 1);
  driver._receive({ type: "joined", playerId: "p4", resumeToken: "tok2" });
  await driver._receive({ ...lobby, you: null });
  await driver._receive(lobby);
  assert.equal(sent.filter((m) => m.type === "join").length, 2, "losing the seat re-joins once at the next lobby");
  await driver.close();
});
...
test("an AdapterError during a decision rejects done with code adapter; a timeout is a fallback decision event", async () => {
  const { driver, events } = make({ spec: "file:" + path.join(__dirname, "fixtures", "agent-throws.js") });
  driver._receive(joined);
  await driver._receive(decisionState(1));
  await assert.rejects(driver.done, (err) => err instanceof DriverError && err.code === "adapter");
  await driver.close();

  const slow = make({ spec: "bot:threshold25" });
  slow.driver._receive(joined);
  const s = decisionState(1); s.timer = { remainingMs: 251 };   // timeoutMs = 1: even a bot cannot answer in time
  await slow.driver._receive(s);
  const d = slow.events.find((e) => e.type === "decision");
  assert.ok(d, "a decision event is emitted");
  assert.equal(d.turnNumber, 1);
  assert.ok(["timeout", null].includes(d.fallback));
  assert.equal(slow.sent.filter((m) => m.type === "act").length, 1);
  await slow.driver.close();
});
```

Also create `tests/fixtures/agent-throws.js` (an in-process module whose `act` throws; `decide` wraps that into an `AdapterError`, which the driver treats as fatal):
...
// once per turnNumber per game. A driver that is not seated (a visitor: game in progress, or
// its resume token was rejected) watches without acting and joins at the next lobby.
//
...
function driveLiveSeat({ url, room, spec, name, log = console.log, onEvent = null, _send = null }) {
  ...
  let joinInFlight = false, waitingForLobby = false, settled = false;
  const acted = new Set();
  ...
  function send(obj) {
    if (_send) { _send(obj); return; }
    if (ws && ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify(obj));
  }

  // The only place a join is sent.
  function requestSeat() {
    if (seated || joinInFlight || settled) return;
  ...
  async function onState(msg) {
    ...
    else if (msg.you === null && seated) { loseSeat(); waitingForLobby = true; }
    latest = msg;
    if (busy) return;
    busy = true;
    try {
  ...
  function onMessage(msg) {
    if (msg.type === "joined") {
      seated = true; joinInFlight = false; waitingForLobby = false; you = msg.playerId; token = msg.resumeToken;
    ...
    if (msg.type === "error") {
      ...
      if (code === "unknown_token") { loseSeat(); joinInFlight = false; requestSeat(); return undefined; }
...
Expected: all pass. The existing `live.test.js` game still completes: the driver now joins through `requestSeat()` on open and is seated by `joined` before any `playing` state.
...
```bash
git add bench/live.js tests/live-unit.test.js tests/fixtures
git commit -m "Live driver: seat-aware lifecycle, one join path, typed fatal errors, structured events"
```
...
**Files:**
- Modify: `tests/live.test.js` (append three tests; reuse its `PORT` and the human-socket pattern)

**Interfaces:**
- Consumes: `driveLiveSeat` events and `_token()` from Task 4.

- [ ] **Step 1: Add a helper and the three tests** (append after the existing test)

```js
// A human-like socket that seats itself, and exposes the messages it saw.
async function human(room, humanName = "Kay") {
  ...
  return { ws, send, until, close: () => ws.close() };
}
...
test("a driver that connects mid-game waits, then holds exactly one seat after the room returns to the lobby", async () => {
  ...
  const events = [];
  const driver = driveLiveSeat({ url: `ws://127.0.0.1:${PORT}`, room, spec: "bot:threshold25", name: "late", onEvent: (e) => events.push(e) });
  driver.done.catch(() => {});
  await new Promise((resolve) => { const i = setInterval(() => { if (events.some((e) => e.type === "waiting")) { clearInterval(i); resolve(); } }, 20); });
  await h.until((m) => m.type === "state" && m.phase === "game_over", "first game over");
...
test("two drivers in one room both play a full game", async () => {
  const room = "duoo";
  const h = await human(room);
  const a = driveLiveSeat({ url: `ws://127.0.0.1:${PORT}`, room, spec: "bot:threshold25", name: "alpha", log: () => {} });
  const b = driveLiveSeat({ url: `ws://127.0.0.1:${PORT}`, room, spec: "bot:threshold25", name: "beta", log: () => {} });
...
test("a resume from another socket takes the seat over and rejects done with seat_taken_over", async () => {
  const room = "take";
  const events = [];
  const driver = driveLiveSeat({ url: `ws://127.0.0.1:${PORT}`, room, spec: "bot:threshold25", name: "orig", onEvent: (e) => events.push(e) });
  await new Promise((resolve) => { const i = setInterval(() => { if (driver._token()) { clearInterval(i); resolve(); } }, 20); });
  ...
  await assert.rejects(driver.done, (err) => err.code === "seat_taken_over");
...
Expected: 4 pass. The mid-game test plays one full game first (about 20 to 40 s with the 2 s turn timer and instant bots); if it exceeds 60 s, raise only that test's `until` limit to 90 s.

- [ ] **Step 3: Commit**

```bash
git add tests/live.test.js
git commit -m "Live driver integration tests: mid-game join, two drivers, seat takeover"
```
...
test("SIGINT during a game exits 130 after shutdown", { skip: process.platform === "win32" && "Windows cannot deliver SIGINT to a child" }, async () => {
  ...
  await new Promise((resolve) => child.stdout.on("data", (b) => { if (/seated as/.test(String(b))) resolve(); }));
  const started = Date.now();
  child.kill("SIGINT");
...
- [ ] **Step 3: Write `bin/flip7-agent.js`** (LF line endings; the Write tool produces LF, verify with `file bin/flip7-agent.js` showing no "CRLF")
...
let stopping = false;
function stop(code) {
  if (stopping) { process.exit(code); return; }
  stopping = true;
  setTimeout(() => process.exit(code), 3000).unref();
  driver.close().then(() => process.exit(code), () => process.exit(code));
}
...
Check `render.yaml` and `server.js` do not reference the package name or `main` (grep `flip7-arena` in `render.yaml` refers to the service name, which is unrelated). `npm start` still runs `node server.js`.
...
Run: `file bin/flip7-agent.js` → must not say CRLF. `git ls-files --eol bin/flip7-agent.js` → `i/lf w/lf`.

- [ ] **Step 7: Commit**

```bash
git add bin/flip7-agent.js package.json LICENSE .gitattributes tests/bin.test.js
...
**Files:**
- Modify: `docs/agent-protocol.md` (new §0 before "## 1. Adapters"; new row in the adapter table; rewrite "## 7. Live-room driver")
- Modify: `README.md` (the "An agent can also take a seat" paragraph and its code block; add a line under "Benchmark quick start")
...
- [ ] **Step 2: Adapter table row** — add after the `http://host:port` row:
...
`driveLiveSeat({ url, room, spec, name, onEvent })` connects to `<url>/ws?room=<room>`, calls `hello` once, then requests a seat with `join { name, agent: true }` through a single guarded path. If the room is mid-game the server answers `game_in_progress`; the driver waits as a visitor, never calling `start` or acting, and joins at the next lobby state. A rejected resume token (`unknown_token`) clears its identity and requests a fresh seat. It calls the adapter's `start` hook exactly once per game, on the first `playing` state seen while seated since the last lobby or `game_over` (this also covers joining or reconnecting mid-game), and `end` exactly once on `game_over`. On every `state` message where `game.decision` is set, `game.current_player` is this seat, and the turn has not already been answered, it calls `decide` with `timeoutMs = max(1, timer.remainingMs − 250)` and retry disabled (a human or another agent is waiting, so there is no second attempt), then sends `act { turnNumber, action }`. States that arrive while a decision is still in flight are not dropped: the newest one is reprocessed once the current decision settles, so a turn the server already defaulted is never answered late. It reconnects with the same backoff as the browser (1 s × 1.5, capped at 5 s) on every socket close except code `4000`. `close()` cancels any pending reconnect and awaits the adapter's `shutdown`.
...
- [ ] **Step 2: Claude Code seat.** Same with `--agent claude-code` (run with the `CLAUDECODE` env var present, since this session is inside Claude Code; the adapter strips it). Confirm `agent claude-code:<model>@<version> ready` appears within ~10 s, every turn is answered inside the 30 s ring, and latency is printed.
- [ ] **Step 3: Two drivers.** Threshold and claude-code in one room, browser starts; both exit 0.
- [ ] **Step 4: Ctrl-C mid-decision.** Start a claude-code driver, and while its turn is thinking, send Ctrl-C (from a Bash tool call, kill the driver with `powershell.exe -Command "Stop-Process -Id <pid>"` is not a SIGINT; instead run the driver in a foreground PowerShell window if available, or verify via the unit test in Task 3 and note it in the PR). Confirm no `claude` process remains: `tasklist | grep -i claude`.
