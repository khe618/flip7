...
// MODE=huge answers a single stdout line over 64 KB on the first move then stay;
// MODE=hang-shutdown keeps the event loop alive after hello so shutdown() must SIGKILL it.
...
rl.on("line", (line) => {
  ...
  if (msg.type === "hello") {
    ...
    if (mode === "hang-shutdown") setInterval(() => {}, 1000);
    return;
  }
  ...
  if (mode === "huge" && moves === 1) {
    process.stdout.write(JSON.stringify({ request_id: msg.request_id, action: legal.includes("stay") ? "stay" : legal[0], pad: "x".repeat(70000) }) + "\n");
    return;
  }
